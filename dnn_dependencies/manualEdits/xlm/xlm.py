...
def main() -> None:
    """ """
    graph: DiGraph = read_gexf("xlm.gexf")

    attentions_LayerNodes: defaultdict[str, List[str]] = findRelevantNodes(
        graph=graph,
        pattern=r"attentions\.(\d+)",
    )

    layerNormNum_LayerNodes: defaultdict[str, List[str]] = findRelevantNodes(
        graph=graph, pattern=r"layer_norm(\d+)"
    )
    ...
    #     condensedGraph: DiGraph = condenseLayers(graph=graph, layerNodes=layerNormNum_LayerNodes)
    condensedGraph: DiGraph = condenseLayers(
        graph=graph, layerNodes=attentions_LayerNodes
    )
